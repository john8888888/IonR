...
#  fi
  echo "<h1><center><span style=\"cursor:help\" title=\"${ALIGNEDREADS}Library Type='${PLUGINCONFIG__LIBRARYTYPE_ID}'${TARGETS}${OPTIONS}\">AmpliCat Report</span></center></h1>" >> "$1"
}
...
write_page_header ()
{
  ...
  # add specific or generic HTHL header code here
  if [ "$HEADFILE" != "-" ];then
    cat "$HEADFILE" > "$HTML"
  fi
  ##### add extra stuff, e.g. javascript includes, here ####
  echo '</head>' >> "$HTML"
  echo '<body>' >> "$HTML"
...
write_html_header ()
{
  ...
  if [ "$HTML_TORRENT_WRAPPER" -eq 1 ]; then
    echo '<title>Torrent AmpliCat Report</title>' >> "$HTML"
    echo '<body>' >> "$HTML"
  print_html_logo >> "$HTML";
  else
    echo '<body>' >> "$HTML"
...
append_to_json_results ()
{
  ...
  local JSONCMD="perl ${SCRIPTSDIR}/amplicat_json.pl -a -I $INDENTLEV $DATASET \"$DATAFILE\" \"$JSONFILE\""
...
write_html_results ()
{
  ...
  PLUGIN_OUT_STATSFILE="${BAMROOT}.stats.txt" ; # also needed for json output
  PLUGIN_OUT_AMPLICAT="${BAMROOT}.amplicat.xls"

  # Links to folders/files required for html report pages (inside firewall)
  run "ln -sf \"${DIRNAME}/html/lifegrid.css\" \"${OUTDIR}/\"";
  run "ln -sf \"${SCRIPTSDIR}/igv.php3\" \"${OUTDIR}/\"";

  # Create the html report page
  echo "(`date`) Publishing HTML report page..." >&2
  local HTMLOUT="${OUTDIR}/${HTML_RESULTS}";
  write_page_header "$LIFECHART/TAC.head.html" "$HTMLOUT";
  cat "${OUTDIR}/$PLUGIN_OUT_COVERAGE_HTML" >> "$HTMLOUT"
  write_file_links "$OUTDIR" "$HTML_RESULTS" "$OUTURL";
