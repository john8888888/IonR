...
PLUGIN_CREATE_BCMATRIX=0;       # 0 to disable any cross-barcode analysis & output
PLUGIN_CREATE_NOFP_BAM=0;	# output BAM file containing just assigned reads that were assume to be due to false-priming
...
# Overrides for options not used yet
PLUGINCONFIG__SAMPLEID="No"
PLUGINCONFIG__TRIMREADS="No"
PLUGINCONFIG__PADTARGETS=0
PLUGINCONFIG__UNIQUEMAPS="No"
...
PLUGIN_TARGETS="$PLUGIN_DETAIL_TARGETS"
PLUGIN_ANNOFIELDS="-f 4,8"
AMPOPT="-a"
if [ "$PLUGIN_RUNTYPE" != "ampliseq" ]; then
  echo "WARNING: AmpliCat plugin is for AmpliSeq assays. Target regions assumed to be suitable." >&2
  PLUGIN_RUNTYPE="ampliseq"
fi
...
if [ -n "$PLUGIN_DETAIL_TARGETS" ]; then
...
elif [ -z "$PLUGIN_BC_TARGETS" ];then
  # Graceful (non-error) exit if no targets file defined
  rm -f "${TSP_FILEPATH_PLUGIN_DIR}/results.json"
  HTML="${TSP_FILEPATH_PLUGIN_DIR}/${PLUGINNAME}.html"
  echo '<html><body>' > "$HTML"
  if [ -f "${DIRNAME}/html/logo.sh" ]; then
  ...
  echo "<h3><center>${PLUGIN_RUN_NAME}</center></h3>" >> "$HTML"
  echo "<br/><h2 style=\"text-align:center;color:red\">*** Automatic analysis was not performed. ***</h2>" >> "$HTML"
  echo "<br/><h3 style=\"text-align:center\">Amplicon targets file must be specified.</h3></br>" >> "$HTML"
  echo '</body></html>' >> "$HTML"
  echo "ERROR: This plugin requires an amplicon targets file." >&2
  exit 0
fi

# Definition of file names, etc.
LIFECHART="${DIRNAME}/html"
...
BC_COL_TITLE[0]="Valid Reads"
BC_COL_HELP[0]="Number of assigned reads that aligned that do not extend more than 30 bases outside of their amplicon inserts."
BC_COL_TITLE[1]="Low Quality"
BC_COL_HELP[1]="Percentage of amplicons with 20% insert reads were aligned SAM MAPQ score <= 10."
...
BC_COL_TITLE[3]="Low Read-thru"
BC_COL_HELP[3]="Percentage of amplicons without significant strand bias that had low coverage over the forward and/or reverse target strand."
BC_COL_TITLE[4]="False Primed"
BC_COL_HELP[4]="Percentage of amplicons that had a least 20% of their reads appearing to be short due to mispriming."
...
rm -f ${TSP_FILEPATH_PLUGIN_DIR}/*.stats.txt ${TSP_FILEPATH_PLUGIN_DIR}/*.xls ${TSP_FILEPATH_PLUGIN_DIR}/*.png
rm -f ${TSP_FILEPATH_PLUGIN_DIR}/*.bam* ${TSP_FILEPATH_PLUGIN_DIR}/*.bed*
rm -rf ${TSP_FILEPATH_PLUGIN_DIR}/scraper
...
if [ -n "$PLUGIN_DETAIL_TARGETS" ];then
  run "ln -sf \"$PLUGIN_DETAIL_TARGETS\" ${TSP_FILEPATH_PLUGIN_DIR}/.";
fi
GCANNOBED=$PLUGIN_DETAIL_TARGETS
...
if [ $PLUGIN_CHECKBC -eq 1 ]; then
  ...
  if [ "$PLUGIN_CREATE_NOFP_BAM" -ne 0 ];then
    BAMOUTOPT="-F \"${PLUGIN_RUN_NAME}.noFP.bam\""
  fi
  run "ln -sf \"${TESTBAM}\" \"${PLUGIN_RUN_NAME}.bam\""
  run "ln -sf \"${TESTBAM}.bai\" \"${PLUGIN_RUN_NAME}.bam.bai\""
  # Run on single bam
  RT=0
  eval "${SCRIPTSDIR}/run_amplicat.sh $LOGOPT $FILTOPTS $AMPOPT $TRIMOPT $BAMOUTOPT -R \"$HTML_RESULTS\" -D \"$TSP_FILEPATH_PLUGIN_DIR\" -A \"$GCANNOBED\" -B \"$PLUGIN_EFF_TARGETS\" -C \"$PLUGIN_TRGSID\" -P \"$PADDED_TARGETS\" -p $PLUGIN_PADSIZE -Q \"$HTML_BLOCK\" -S \"$PLUGIN_SAMPLEID_REGIONS\" \"$REFERENCE\" \"${PLUGIN_RUN_NAME}.bam\"" || RT=$?
  if [ $RT -ne 0 ]; then
  ...
  write_html_results "$PLUGIN_RUN_NAME" "$TSP_FILEPATH_PLUGIN_DIR" "$TSP_URLPATH_PLUGIN_DIR" "${PLUGIN_RUN_NAME}.bam"
  # Write json output
  write_json_header;
