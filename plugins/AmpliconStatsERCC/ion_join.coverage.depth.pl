...
my $originalBed = $ARGV[7] || die "Usage: for.depth.file rev.depth.file joined.depth.outfile bias.base.outfile merged.bed.file chr.stats.outfile offTargetOn original.bed.file\n";
...
sub write1 {
    #return if (!$in1[0] || $in1[0] == 0);
    return if (!$in1[0] || $in1[0] eq '');
...
sub write2 {
    #return if (!$in2[0] || $in2[0] == 0);
    return if (!$in2[0] || $in2[0] eq '');
...
sub writeboth {
    #return if ($in1[0] == 0 && $in2[0] == 0);
    return if ($in1[0] eq '' && $in2[0] eq '');
    my $sum = $in1[2] + $in2[2];
...
sub decideWriteWhichOne {
	...
	&readOriginalBed() if ($orgBedRead == 0);
	$c1num = $refOrder{$c1};
	$c2num = $refOrder{$c2};
    }
...
sub readOriginalBed {
    my $ct = 1;
    open (OB, $originalBed) || die "Can't open $originalBed: $!\n";
    while (<OB>) {
	next if (/^track/);
	my @w = split(/\t/, $_);
	$refOrder{$w[0]} = $ct;
