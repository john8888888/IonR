BoxMTest <- function(X, cl, alpha=0.05) { 
  ...
  ## 
  ## Inputs: 
  ## X - data matrix (Size of matrix must be n-by-(1+p); sample=column1, 
  ...
  #######Modified by Nianxiang Zhang to accomodate situation when any n=<p
if(p>=min(n)){
    mn<-min(n)
   require(ClassDiscovery)
  temp.pca<-SamplePCA(t(X))
  temp.pctv<-cumsum(temp.pca@variances)/sum(temp.pca@variances)
  if(temp.pctv[mn-1]<0.95) warnings('sample size in one group is too small')  
  X<-temp.pca@scores[,1:(mn-2)]
  p<-ncol(X)
}
  #########end of modification
  ...
  for (k in 1:g) suma = suma + (n[k] - 1) * covList[[k]] 
  Sp = suma / deno ## Pooled covariance matrix. 
  Falta=0 
  for (k in 1:g) Falta = Falta + ((n[k] - 1) * log(det(covList[[k]]))) 
